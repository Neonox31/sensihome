...
module.exports = internals.Plugins = function (sensihome) {
...
internals.Plugins.prototype.getInstalled = function (cb) {
    ...
    if (this.sensihome.config.get("plugins:autostart") === true) {
        async.waterfall([
            ...
            function (npm, next) {
                npm.commands.ls([], true, next);
            },
            function (data, lite, next) {
                self._plugins = {};

                for (var dependency in data.dependencies) {
                    if (pluginPattern.test(data.dependencies[dependency].name)) {
                        this.sensihome.logger.info("installed plugin detected : %s", data.dependencies[dependency].name);
                ...
                if (_.isEmpty(self._plugins)) {
                    this.sensihome.logger.info("no installed plugins detected.");
                }
                next();
            }
        ],
...
internals.Plugins.prototype.initAll = function (cb) {
    if (_.isEmpty(this._plugins)) {
        this.sensihome.logger.info("initAll : no installed plugins detected.");
        return cb();
    }
    var self = this;
...
internals.Plugins.prototype.init = function (plugin, cb) {
    ...
    var self = this;
    this._plugins[plugin].instance.init.call(this, function (err) {
        if (err)
            return cb(err);
...
internals.Plugins.prototype.runAll = function (cb) {
    if (_.isEmpty(this._plugins)) {
        this.sensihome.logger.info("runAll : no installed plugins detected.");
        return cb();
    }
    var self = this;
...
internals.Plugins.prototype.run = function (plugin, cb) {
    ...
    if (this._plugins[plugin].instance.attributes.config.get("enabled") === false) {
        this.sensihome.logger.info("plugin " + plugin + " not enabled. see its config.");
        return cb();
    }
    var self = this;
    this._plugins[plugin].instance.run.call(this, function (err) {
        if (err)
            return cb(err);
...
internals.Plugins.prototype.stopAll = function (cb) {
    if (_.isEmpty(this._plugins)) {
        this.sensihome.logger.info("stopAll : no installed plugins detected.");
        return cb();
    }
    var self = this;
